...
//! consistent with `VerifyBarycentricEvaluation` as defined in the EIP-4844 spec.
//!
...
//!   `Y = (Z^N - 1) / N * sum_i( d_i * w_i / (Z - w_i) )`
//!
//! `w_i` are the 4096 EIP-4844 roots of unity in BLS12-381 Fr, in the
//! bit-reversed order used by go-ethereum / go-eth-kzg.
//!
//! # Performance
//!
...
///   `Y = (z^N - 1) / N * sum_i( d_i * w_i / (z - w_i) )`
/// ## Early exit
/// When `z = w_k` for some k, the formula degenerates and `blob[k]` is returned
...
pub fn evaluate_barycentric(blob: &[u8], z: BlsFrRaw) -> BlsFrRaw {
    ...
    // Differences: z - w_i  (none is zero => guaranteed by early exit)
    ...
    // Barycentric sum: sum_i( d_i * w_i * (z - w_i)^-1 )
    let mut sum = ZERO;
    for i in 0..N {
